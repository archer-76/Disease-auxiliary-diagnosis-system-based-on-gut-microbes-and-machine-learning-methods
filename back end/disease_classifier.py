...
def basic_model_inferrence(para_dict: dict, diagnosizing: bool = False):
    ...
    for i in range(runs_round):
        ...
        for train_index, test_index in skf_split:
            ...
            train_data = evaluation_data[:evaluation_num].loc[i_tr]
            train_label = evaluation_label[:evaluation_num].loc[i_tr]

            train_data = train_data.append(train_data[evaluation_num:])
            train_label = train_label.append(train_label[evaluation_num:])

            val_data = evaluation_data[:evaluation_num].loc[~i_tr]
            val_label = evaluation_label[:evaluation_num].loc[~i_tr]
